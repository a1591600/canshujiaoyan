	...
	# 打开文件
		...
		try:
			...
			try:
				...
				# a = {
				# 	"applicationId": "2333",
				# 	"appliedTime": "2018-01-01 00:00:00",
				# 	"reactTime": "2018-01-01 00:00:00",
				# 	"reactType": "1",
				# 	"reactSymptom": "123",
				# 	"handleTime": "2018-01-01 00:00:00",
				# 	"handleBy": "2018-01-01 00:00:00",
				# 	"patient": {
				# 		"id": "2333333333333",
				# 		"name": "张三",
				# 		"sex": "MALE",
				...
				try:
					...
					try:
						time.strptime(appliedTime, date_format)
					# print("第%s袋血appliedTime数据格式不正确"% i)
					except ValueError:
						print("appliedTime数据格式不正确")
				except:
					print("aappliedTime字段")


				try:
					reactTime = a["reactTime"]
					date_format = "%Y-%m-%d %H:%M:%S"
					try:
						time.strptime(reactTime, date_format)
					# print("第%s袋血reactTime数据格式不正确"% i)
					except ValueError:
						print("reactTime数据格式不正确")
				except:
					print("缺少reactTime字段")
				...
				try:
					...
					reactType = str(reactType)
					reactType = reactType.isdigit()
					if reactType != True:
						print("袋血reactType数据格式不正确")
				except:
					print("缺少reactType字段")


				try:
					reactSymptom = a["reactSymptom"]
					if reactSymptom is None or reactSymptom == "":
						print("reactSymptom值为空")
				except:
					print("缺少reactSymptom字段")
				...
				try:
					...
					date_format = "%Y-%m-%d %H:%M:%S"
					try:
						time.strptime(handleTime, date_format)
					# print("第%s袋血handleTime数据格式不正确"% i)
					except ValueError:
						print("handleTime数据格式不正确")
				except:
					print("缺少handleTime字段")

				try:
					handleBy = a["handleBy"]
					date_format = "%Y-%m-%d %H:%M:%S"
					try:
						time.strptime(handleBy, date_format)
					# print("第%s袋血handleBy数据格式不正确"% i)
					except ValueError:
						print("handleBy数据格式不正确")
				except:
					print("缺少handleBy字段")
