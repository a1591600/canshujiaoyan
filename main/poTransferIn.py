...
def poTransferIn():
	path1 = input("请输入文件的绝对路径如如：C:\\Users\\Administrator\\Desktop\\ceshi.txt：")
	path =path1
	try:
		# 打开文件
		...
		try:
			...
			try:
				...
				try:
					...
					transferFrom = str(transferFrom)
					transferFrom = transferFrom.isdigit()
					if transferFrom != True:
						print("第%s袋血transferFrom数据格式不正确" )
				except:
					print("缺少transferFrom字段")
