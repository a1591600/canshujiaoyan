		...
		try:
			...
			try:
				...
				try:
					...
					try:
						time.strptime(scrapDate, date_format)
					# print("第%s袋血scrapDate数据格式不正确"% i)
					except ValueError:
						print("scrapDate数据格式不正确")
				except:
					print("缺少scrapDate字段")
				...
				try:
					...
					scrapReason = str(scrapReason)
					scrapReason = scrapReason.isdigit()
					if scrapReason != True:
						print("袋血scrapReason数据格式不正确")
				except:
					print("缺少scrapReason字段")
